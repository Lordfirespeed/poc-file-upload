...
/* This syntax is functionally equivalent but allows for multiple file fields if you like. */
api_router.route("/avatar_different")
    .post(
        ...
        (request, response) => {
            let tmp_path = request.files.avatar[0]; // since there are multiple uploads, each field has an array of files
            console.log(tmp_path);
            console.log(request.body);
            response.json({status: 200, message: "coolio"});
        }
